-- ============================================================================
-- 0152 — past_due_lapse: the schema half of PR #74, committed after the fact.
--
-- ⚠️ ALREADY APPLIED TO PRODUCTION. This ran against znlfwbnvhlacwzgfalcf on
-- 2026-08-22 (ledger name `past_due_lapse`, version 20260822163838), ahead of
-- the code it was written for — and that code never merged. PR #74 "Write off a
-- customer whose payments keep failing" is still open at the time of writing,
-- so production has carried four nullable columns and two settings rows
-- (`past_due_lapse_enabled = 'true'`, `past_due_lapse_days = '3'`) that nothing
-- reads, and the repository has carried no record of them. §58 records the
-- audit that found it, and PR 3 of that plan re-implements the code half on
-- today's `main`.
--
-- Committed VERBATIM from the branch (claude/cancelled-subscription-status-sync-
-- i2tfat, file 0097_past_due_lapse.sql — that number was long since taken,
-- §56.5) so a rebuild from this directory matches production, the
-- `0100a_worked_conversion` precedent (§36.8). Every statement is idempotent:
-- `add column if not exists`, `on conflict do nothing`, and backfills guarded on
-- `is null`. Re-applying it to production is a no-op and is NOT needed.
--
-- The header below is the original author's, kept in full because it is the
-- design record for the code PR 3 restores.
-- ============================================================================
