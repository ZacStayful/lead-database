 ...
 * 'cancelled' (§23.9) and clears both 0152 columns.
 *
...
async function handle(request: NextRequest) {
  ...
  // Three outcomes, never two (§18.3). The settings are read BY NAME, so an
  // empty result is the ordinary shape of a database where 0152 was never
  // applied and "off" is the right reading — but a failed read must never be
  // reported as a switch somebody turned off. That is a 500, so the cron run
  // is marked failed rather than logged as a decision nobody took.
  const { data: settingRows, error: settingsError } = await admin
    .from("system_settings")
    .select("key, value")
    .in("key", ["past_due_lapse_enabled", "past_due_lapse_days"]);
  const gate = resolveSettingsGate(
    settingRows as SettingsRow[] | null,
    settingsError
  );
  if (!gate.ok && gate.reason === "read_failed") {
    console.error("[lapse-past-due] system_settings unreadable", {
 ...
 *
 * The Customer end date cell is left to the label rule's ordinary machinery:
 * this call passes no endDate, so the cell is untouched here. §23.3 records that
 * the board automation stamping it on entry into Cancelled is still live and
 * still wrong, and that the app is the intended owner; a lapse has no future
 * period end to write, and "today" is what the automation will stamp anyway.
 *
