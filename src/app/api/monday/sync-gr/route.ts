...
import { fetchGuaranteedRentLeads } from "@/lib/monday";
 ...
 * (idempotent). Same authorisation and behaviour as /api/monday/sync — an admin
 * session (the admin "Sync GR from Monday" button) or a CRON_SECRET bearer
...
async function handle(request: NextRequest) {
  ...
  let leads;
  try {
    leads = await fetchGuaranteedRentLeads();
  } catch (err) {
