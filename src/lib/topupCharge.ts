...
export function topupIneligibilityReason(
  ...
  // A failed payment is refused too, and this was a real hole (0152, §59): every
  // check in this function read account_status, which a decline never changes,
  // while the thing that actually stops delivery — both candidate functions
  // requiring subscription_status = 'active' — is the column below. So a
  // declined customer passed all of these and could be charged £75 for credit
  // that routing would never spend, which is the one outcome the doc comment
