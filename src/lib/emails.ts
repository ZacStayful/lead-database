...
/**
 * Cycle-end lead quality survey invite.
 *
 * Sent once per billing cycle when the customer's leads for that cycle have
 * been delivered. Credit-neutral by design: it asks about quality, never about
 * refunds, so the answers are not shaped by anything the customer stands to
 * gain.
 */
export async function sendCycleSurveyEmail(params: {
  to: string;
  contactName: string;
  leadsInCycle: number;
  cycleStart: string;
}): Promise<{ id: string | null; error: unknown }> {
  const { to, contactName, leadsInCycle } = params;
  const subject = `How were your last ${leadsInCycle} leads?`;
  const inner = `
    <h1 style="margin:0 0 4px;font-size:18px">Two minutes on lead quality</h1>
    <p style="margin:0 0 18px;color:#6b706a;font-size:14px">Hi ${esc(contactName)}, you have now had <strong>${leadsInCycle}</strong> lead${leadsInCycle === 1 ? "" : "s"} from us this cycle. Telling us how they went is the fastest way we can improve what you get next month — it takes about two minutes and there are four questions.</p>
    ${button(`${APP_URL}/dashboard`, "Rate this month's leads")}
  `;
  try {
    const { data, error } = await getResend().emails.send({
      from: fromAddress(),
...
/**
 * A dead-lead claim has landed in the review queue. Goes to the Stayful team,
 * not the customer — the customer already got "we're reviewing this".
 */
export async function sendQualityClaimReviewEmail(params: {
  businessName: string;
  leadName: string;
  reason: string;
  detail: string;
  ...
  const { businessName, leadName, reason, detail, corroboration } = params;
  const subject = `[Lead quality] ${esc(businessName)} reported ${esc(leadName)}`;

  const row = (k: string, v: string) =>
    `<tr><td style="padding:6px 0;color:#6b706a;font-size:13px;width:150px;vertical-align:top">${k}</td><td style="padding:6px 0;font-size:14px">${v}</td></tr>`;

  const contradicted = corroboration === "peer_contradicts";
  const inner = `
    <h1 style="margin:0 0 4px;font-size:18px">A lead has been reported</h1>
    <p style="margin:0 0 18px;color:#6b706a;font-size:14px">${
      contradicted
        ? "Another operator on this lead has it live, so this one needs a look before any credit goes out."
        : "This one is outside what we settle automatically, so it needs a decision."
    }</p>
    <table style="width:100%;border-collapse:collapse;margin-bottom:18px">
      ${row("Operator", esc(businessName))}
      ${row("Lead", esc(leadName))}
      ${row("Reason", esc(reason))}
      ${row("What they said", esc(detail))}
    </table>
    ${button(`${APP_URL}/admin/quality`, "Review the claim")}
  `;
  try {
    const { data, error } = await getResend().emails.send({
      from: fromAddress(),
...
/**
 * Tell a customer how their reported lead was settled.
 *
 * Deliberately says nothing about allowances, budgets or how many reports they
 * have left. The upheld copy says what happened; the declined copy gives the
 * reviewer's reason so the decision is never a black box.
 */
export async function sendQualityClaimResolvedEmail(params: {
  to: string;
  leadName: string;
  upheld: boolean;
  ...
  const { to, leadName, upheld, resolution, reviewNote } = params;
  const subject = upheld
    ? `We've sorted out ${leadName}`
    : `About the lead you reported`;

  const outcome =
    resolution === "replacement"
      ? "We have sent you a replacement lead to make up for it."
      : "We have credited it back to your balance, so it will come round again as a new lead.";

  const inner = upheld
    ? `
    <h1 style="margin:0 0 4px;font-size:18px">Thanks for telling us</h1>
    <p style="margin:0 0 18px;color:#6b706a;font-size:14px">We looked into <strong>${esc(leadName)}</strong> and you were right. ${outcome}</p>
    ${button(`${APP_URL}/dashboard`, "View your leads")}
  `
    : `
    <h1 style="margin:0 0 4px;font-size:18px">We looked into that lead</h1>
    <p style="margin:0 0 12px;color:#6b706a;font-size:14px">We reviewed <strong>${esc(leadName)}</strong> and it stays on your account this time.</p>
    ${reviewNote ? `<p style="margin:0 0 18px;color:#6b706a;font-size:14px">${esc(reviewNote)}</p>` : ""}
    <p style="margin:0 0 18px;color:#6b706a;font-size:14px">If you think we have got that wrong, reply to this email and we will take another look.</p>
    ${button(`${APP_URL}/dashboard`, "View your leads")}
  `;
