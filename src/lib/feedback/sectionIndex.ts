...
export const KNOWN_ISSUES: string[] = [
  ...
  "⚠️ isRetired() in filterPrediction.ts mirrors only PART of the retirement predicate, by design (it is a JS copy of the two leads-column branches plus the pool-claim set, because the SQL function is s…",
  "⚠️ withdrawn_slots_per_month on the capacity panel now includes Stayful-pipeline withdrawals (§64) as well as swaps, because the flag function writes withdrawn_slots through the same column 0145 read…",
