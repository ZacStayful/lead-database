...
## 60. Measuring the adverts *(no migration)*

⚠️ **This is PR #89, restored.** It was opened and closed unmerged within 22
minutes on 2026-09-04 with `mergeable_state: clean`, because its CLAUDE.md
section collided with the OAuth work that took §45 the same afternoon. §58.4
found it. Its one commit is cherry-picked onto today's `main` with four
conflicts resolved by hand: the enquiry route now fires the server `Lead`
AFTER `recordEnquiry()` returns and in the route rather than inside the shared
helper (the Monday enquiry sync of §57 calls that helper for a Facebook
lead-form enquiry, which never touched this site and has no browser event to
deduplicate against); the enquiry page keeps `lead_interest` (§47) and adds
`event_id`; the privacy policy keeps the Meta transfer bullet and not the
ZeroBounce one §51.11 removed; and the section is numbered 60. Everything
below is the original text with its section references renumbered.
...
### 60.1 — The server half is the RELIABLE one, not a backup
...
### 60.2 — Inert until configured, and that is asserted
...
### 60.3 — Four things that fail SILENTLY
...
### 60.4 — Phone normalisation is `normaliseUkMobile`, not a third rule
...
### 60.5 — `usePathname`, never `useSearchParams`
...
### 60.6 — `_fbc` is minted from `?fbclid=` by OUR bundle
...
### 60.7 — Purchase, and what it cannot tell you
...
### 60.8 — The pixel does not load on `/dashboard` or `/admin`
...
### 60.9 — No barrel in `src/lib/meta/`
...
### 60.10 — The privacy policy, and what a rewrite does NOT buy
